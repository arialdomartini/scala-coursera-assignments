...
object Huffman {
  ...
  */
  ...
  def countChar(char: Char, list: List[(Char, Int)]): List[(Char, Int)]  = {
    if(list.isEmpty) List((char, 1))
    else {
      /*list.head match {
        case (char, count) => (char, count + 1) :: list.tail
        case (_, _) => list.head :: countChar(char, list.tail)  
      }
      */
      if(list.head._1 == char) (char, list.head._2 + 1) :: list.tail
      else list.head :: countChar(char, list.tail)
    }
  }
  ...
  */
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decode(tree: CodeTree, bits: List[Bit], root: CodeTree, list: List[Char]): List[Char] = {
      tree match {
        case Leaf(char, _) => {
          if(bits.isEmpty) char :: list
          else char:: decode(root, bits, root, list)
        }
        case Fork(left, right, _, _) => {
          if(bits.head == 0) decode(left, bits.tail, root, list)
          else decode(right, bits.tail, root, list)
        }
      }
    }
    decode(tree, bits, tree, List())
  }
